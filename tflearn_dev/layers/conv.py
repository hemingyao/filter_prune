...
def conv_3d(incoming, nb_filter, filter_size, strides=1, padding='same',
    ...
    with tf.variable_scope(scope, default_name=name, values=[incoming],
        ...
        scale_decay = tf.multiply(tf.norm(scale, 1), 0.1, name='scale_loss')
        tf.add_to_collection('losses', scale_decay)


        inference = tf.nn.conv3d(incoming, tf.multiply(W, scale), strides, padding)
...
def conv_3d_old(incoming, nb_filter, filter_size, strides=1, padding='same',
            activation='linear', bias=True, weights_init='uniform_scaling',
            bias_init='zeros', regularizer=None, weight_decay=0.001,
            trainable=True, restore=True, reuse=False, scope=None,
            name="Conv3D"):
    ...
    with tf.variable_scope(scope, default_name=name, values=[incoming],
        ...
        with tf.variable_scope('scale'):
            ...
            W_flatten = tf.reshape(W, [-1, out_dim])
            mag = tf.norm(W_flatten, 1, axis=0, keep_dims=True)
            out = _fully_connected(mag, out_dim/4, 'fc1')
            out = tf.nn.relu(out)
            out = _fully_connected_rescale(out, out_dim, 'fc2')
            #out = tflearn.batch_normalization(out, scope='bn')
            scale = tf.nn.sigmoid(out)
...
def residual_bottleneck(incoming, nb_blocks, bottleneck_size, out_channels,
    ...
    with tf.variable_scope(scope, default_name=name, values=[incoming],
        ...
        for i in range(nb_blocks):
            ...
            resnet = conv_2d(resnet, bottleneck_size, 1, 1,
                             'valid',
                             'linear', bias, weights_init,
                             bias_init, regularizer, weight_decay,
                             trainable, restore)

            resnet = max_pool_2d(resnet, 2, strides=2)
